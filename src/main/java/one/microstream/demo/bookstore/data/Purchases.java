...
public interface Purchases
{
	 ...
	 * @see #add(Purchase)
	 */
	public void add(Purchase purchase, PersistenceStoring persister);
	
	public void addAndRemoveRandom(Purchase purchase, PersistenceStoring persister);
	...
	public static class Default extends ReadWriteLockedStriped.Scope implements Purchases
	{
		...
		static class YearlyPurchases
		{
			...
			YearlyPurchases addAndRemoveRandom(
				final Purchase purchase,
				final PersistenceStoring persister
			)
			{
			...
			private static <K> void addAndRemoveFromMap(
				final Map<K, Lazy<List<Purchase>>> map,
				final K key,
				final Purchase purchase,
				final K keyToRemove,
				final Purchase toRemove,
				final List<Object> changedObjects
			)
			{
				addToMap(map, key, purchase, changedObjects);
				final List<Purchase> list = Lazy.get(map.get(keyToRemove));
				if(list != null)
				{
		...
		@Override
		public void addAndRemoveRandom(final Purchase purchase, final PersistenceStoring persister)
		{
			final Integer year = purchase.timestamp().getYear();
			this.write(year, () ->
			{
				...
				if(lazy != null)
				{
					lazy.get().addAndRemoveRandom(purchase, persister);
				}
				else
				{
					this.write(0, () -> {
